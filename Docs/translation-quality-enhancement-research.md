...
### 5.4 基线评估结果（2026-04-12）

使用 Ars Technica 文章 *"RFK Jr. rewrites CDC panel's charter, opening door to anti-vaccine quacks"* 的前 4 段作为测试样本。翻译由 sonnet 按照后端完全相同的 prompt 模板执行（含文章上下文注入 + 滑动窗口），评估由 haiku judge 完成。

**管线配置**：Phase 1 已落地 — 文章标题/来源注入 system prompt + 首段钉住滑动窗口（5 pairs / 2400 chars）

#### 逐段评分

| 段落 | faithfulness | fluency | terminology | 评审意见 |
|------|-------------|---------|-------------|----------|
| P1 | 5 | 4 | 5 | "反疫苗立场鲜明的卫生部长"略冗，可缩减 |
| P2 | 5 | 4 | 5 | "本周一，《联邦公报》上的一则公告显示"可更紧凑 |
| P3 | 5 | 4 | 5 | "契合"一词重复使用 |
| P4 | 5 | 5 | 5 | 无问题 |
...
| 指标 | 得分 | 详情 |
|------|------|------|
| 跨段落术语一致性 | **5/5** | ACIP、章程、肯尼迪、反疫苗、mRNA — 全程零漂移 |
| 总分 | **4.5/5** | 忠实度满分，术语一致性满分，流畅度 P1-P3 有轻微冗余 |

#### 与无上下文管线的对比

| 场景 | terminology 得分 | 典型问题 |
|------|-----------------|----------|
| 无上下文（旧管线） | 2/5 | "consensus protocol" → "共识协议" / "一致性协议" 术语漂移 |
| Phase 1 管线（当前） | **5/5** | 零漂移 |

#### 结论

- **术语一致性**已由上下文注入 + 首段钉住解决，是 Phase 1 的核心收益
- **流畅度**是当前主要扣分点（4/5），属于 Phase 2 Reflect+Refine 的优化目标
- 建议下一步优先实现 Reflect+Refine 可选开关，针对流畅度做二次润色
