...
## 0. 🔒 核心协议前置读取（适用于下方所有情景）
**触发条件**：进入任意协议分支前（1~7 与默认任务执行）。
**行动**：
1. **先读取**：仓库根目录 `.rules`。
2. **再读取**：`.claude/memory/MEMORY_INDEX.md`，并按阶段映射加载对应 memory（`init/coding/docs/release`）。
3. **提交发布场景补充**：涉及 commit/push/release 时，必须额外读取 `.claude/commit_message_policy.md` 与 `.claude/memory/memory_release.md`。
4. **约束**：未完成前置读取，不得执行编码、重构、提交、推送等动作。
